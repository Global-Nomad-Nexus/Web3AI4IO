#!/usr/bin/env python3
"""Audit the immutable application-arm reproducibility bundle without running models."""
...
BUNDLE = ROOT / "data/external/pumpswap/20260810/bundle"
UPSTREAM = BUNDLE / "01_Pumpfun_PumpSwap_Project/pumpfun_pumpswap_did_mvp_full_local"
APPLICATION = BUNDLE / "Web3AI4IO" / "Shilin"
...
def main() -> None:
    datasets = {
        ...
        "solana_rpc_windows": csv_audit(
            APPLICATION / "artifacts/external_validation/solana_post_migration_pool_windows.csv",
            id_column="mint",
            time_columns=("graduated_at", "first_trade_at", "last_trade_at"),
        ),
        "moralis_swaps_raw": csv_audit(
            APPLICATION / "artifacts/external_validation/moralis_token_swaps.csv",
            id_column="mint",
            time_columns=("block_timestamp",),
        ),
        "moralis_fetch_status": csv_audit(
            APPLICATION / "artifacts/external_validation/moralis_fetch_status.csv",
            id_column="mint",
            category_columns=("status",),
        ),
        "moralis_decoded_outcomes": csv_audit(
            APPLICATION / "artifacts/external_validation/moralis_decoded_token_outcomes.csv",
            id_column="mint",
            time_columns=("graduated_at", "first_decoded_trade_at", "last_decoded_trade_at"),
        ),
        "dune_post_migration_trades": csv_audit(
            APPLICATION / "artifacts/external_validation/dune_post_migration_trades.csv",
            id_column="mint",
        ),
        "base_token_created": csv_audit(
            APPLICATION / "artifacts/external_validation/clanker_base_token_created.csv",
            id_column="token_id",
            time_columns=("block_timestamp_utc",),
            category_columns=("clanker_version_class",),
        ),
        "base_full_manifest": csv_audit(
            APPLICATION / "artifacts/external_validation/clanker_base_full_cohort_manifest.csv",
            id_column="token_id",
            time_columns=("launch_timestamp_utc",),
            category_columns=("cohort_side",),
        ),
        "base_horizons": csv_audit(
            APPLICATION / "artifacts/external_validation/clanker_base_token_horizons.csv",
            id_column="token_id",
