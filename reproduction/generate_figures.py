...
from build_teaser import build_teaser
from theme import (
    AMBER,
    BLUE,
    GREEN,
...
# NeurIPS workshop textwidth is 5.5 in. Main-text pairs sit at 0.49\linewidth;
# the appendix pair sits at 0.485\linewidth.
PAIR_WIDTH_IN = 0.49 * 5.5
APP_WIDTH_IN = 0.485 * 5.5
PAIR_HEIGHT_IN = 1.95
SHORT_HEIGHT_IN = 1.50
PAIR_TICK_PT = 6.0
PAIR_LABEL_PT = 7.0
PAIR_NOTE_PT = 6.5
PAIR_HEADER_PT = 6.5
PAIR_MARGINS = dict(left=0.16, right=0.98, top=0.94, bottom=0.18)


def save(fig, stem: str, *, svg: bool = True, native: bool = False) -> None:
    OUT.mkdir(parents=True, exist_ok=True)
    ctx = {"savefig.bbox": None, "savefig.pad_inches": 0.0} if native else {}
...
def _matplotlib_teaser_draft() -> None:
...
def _pair_axes(ax) -> None:
    ax.tick_params(labelsize=PAIR_TICK_PT, length=2.2, width=0.6, pad=1.5)
    ax.xaxis.label.set_size(PAIR_LABEL_PT)
    ax.yaxis.label.set_size(PAIR_LABEL_PT)
    clean_axes(ax)


def make_event_study() -> None:
    d = read_csv("application/event_study_coefficients.csv")
    fig, ax = plt.subplots(figsize=(PAIR_WIDTH_IN, PAIR_HEIGHT_IN))
    fig.subplots_adjust(left=0.205, right=0.985, top=0.935, bottom=0.20)
    ...
    ax.fill_between(pre["rel_week"], pre["ci95_low"], pre["ci95_high"], color=BLUE, alpha=0.18, linewidth=0)
    ax.fill_between(post["rel_week"], post["ci95_low"], post["ci95_high"], color=BLUE, alpha=0.22, linewidth=0)
    ax.errorbar(
        pre["rel_week"],
        pre["coef"],
        yerr=1.96 * pre["std_error"],
        fmt="o",
        color=MUTED,
        markersize=3.6,
        capsize=1.4,
    ...
    ax.scatter(flagged["rel_week"], flagged["coef"], s=42, facecolors="none", edgecolors=RED, linewidths=1.05, zorder=4)
    post_mean = float(post.loc[post["rel_week"] != -1, "coef"].mean())
    ax.axhline(post_mean, color=AMBER, linestyle="--", linewidth=0.9)
    ax.text(0.02, 1.012, "PRE-PERIOD", transform=ax.transAxes, color=MUTED, fontsize=PAIR_HEADER_PT, fontweight="bold", ha="left", va="bottom", clip_on=False)
    ax.text(0.98, 1.012, "POST-PERIOD", transform=ax.transAxes, color=BLUE, fontsize=PAIR_HEADER_PT, fontweight="bold", ha="right", va="bottom", clip_on=False)
    ax.text(0.98, 0.965, f"post mean +{post_mean:.3f}", transform=ax.transAxes, color=AMBER, ha="right", va="top", fontsize=PAIR_NOTE_PT, clip_on=False)
    ax.annotate(
        "pre-trend warning\nweeks -8, -3\nexclude zero",
        xy=(-3, float(d.loc[d["rel_week"] == -3, "coef"].iloc[0])),
        xytext=(-6.55, 1.55),
        color=RED,
        fontsize=PAIR_NOTE_PT,
        ha="left",
        va="center",
        linespacing=1.08,
        arrowprops={"arrowstyle": "->", "color": RED, "lw": 0.65},
    )
    ax.set_xlim(-8.7, 8.7)
    ax.set_ylim(-1.28, 2.68)
    ax.set_xticks([-8, -6, -4, -2, 0, 2, 4, 6, 8])
    ax.set_xlabel("Weeks relative to PumpSwap launch\n(week -1 omitted)", linespacing=1.10)
    ax.set_ylabel("Coefficient on\nlog(1 + daily volume)", linespacing=1.12)
    ax.xaxis.labelpad = 2.2
    ax.yaxis.labelpad = 2.0
    _pair_axes(ax)
    save(fig, "fig_event_study", native=True)
...
def make_ladder() -> None:
    ...
    fig, ax = plt.subplots(figsize=(PAIR_WIDTH_IN, PAIR_HEIGHT_IN))
    fig.subplots_adjust(left=0.205, right=0.985, top=0.935, bottom=0.248)
    bands = [
        (-0.5, 0.5, LIGHT_BLUE),
        (0.5, 2.5, LIGHT_GREY),
        (2.5, 3.5, LIGHT_AMBER),
    ...
    for x0, x1, face in bands:
        ax.axvspan(x0, x1, color=face, alpha=0.7, zorder=0)
    # Title-case labels at 6.5 pt packed across the plot; ALL-CAPS originals do not fit.
    for xpos, align, ink, lab in (
        (0.00, "left", BLUE, "Baseline"),
        (0.219, "left", BLUE, "Identification"),
        (0.552, "left", AMBER, "Dynamics"),
        (1.00, "right", BLUE, "Inference"),
    ):
        ax.text(
            xpos,
            1.012,
    ...
    ax.axhline(0, color=INK, linewidth=0.8)
    ax.fill_between(x, lo, hi, color=BLUE, alpha=0.16, linewidth=0)
    ax.plot(x, est, color=BLUE, linewidth=1.25, marker="o", markersize=4.0, zorder=3)
    ax.text(0.08, 0.90, f"dashboard +{est[0]:.3f}", color=GREEN, fontsize=PAIR_NOTE_PT, ha="left", va="bottom", clip_on=False)
    ax.text(1.5, -0.38, "controls cross zero", color=MUTED, fontsize=PAIR_NOTE_PT, ha="center")
    ax.text(2.95, 1.22, f"dynamic mean +{est[3]:.3f}", color=AMBER, fontsize=PAIR_NOTE_PT, ha="center", va="bottom", clip_on=False)
    ax.text(4.40, -1.18, f"honest interval\nwidth = {hi[4] - lo[4]:.3f}", color=MUTED, fontsize=PAIR_NOTE_PT, ha="right", va="top", clip_on=False)
    ax.set_xticks(
        x,
        [
            "L0\nBefore-\nafter",
    ...
    ax.text(
        0.0,
        -0.205,
        "*L3 uses the dynamic post-event estimate;\nother rungs use the static market effect.",
        transform=ax.transAxes,
        fontsize=6.0,
        color=MUTED,
        va="top",
    ...
    save(fig, "fig_ladder_decision_flip", native=True)
...
def make_metric_battery() -> None:
    ...
    stems = ["#F4C7C3", "#F8E3B0", "#F3D0B0", LIGHT_BLUE]
    fig, ax = plt.subplots(figsize=(PAIR_WIDTH_IN, SHORT_HEIGHT_IN))
    fig.subplots_adjust(left=0.29, right=0.84, top=0.97, bottom=0.12)
    y = np.arange(len(labels))[::-1]
    ax.hlines(y, 0, values, color=stems, linewidth=6.5, zorder=2)
    ax.scatter(values, y, s=38, c=colors, zorder=3, clip_on=False)
    ax.annotate(
        "99.8%",
        xy=(timeout, y[0]),
        xytext=(4, 0),
    ...
    ax.set_xlim(0, 108)
    ax.set_ylim(-0.28, 3.22)
    ax.set_xticks([0, 20, 40, 60, 80, 100])
    _pair_axes(ax)
    ax.grid(False)
    clean_axes(ax, grid_axis="x")
    ax.tick_params(axis="y", pad=2.2)
    for lab in ax.get_yticklabels():
        lab.set_linespacing(0.95)
        lab.set_ha("right")
        lab.set_va("center")
    save(fig, "fig_metric_battery_status", native=True)
...
def make_frequency() -> None:
    ...
    daily_est = float(daily["estimate"])
    weekly_est = float(weekly["estimate"])
    daily_lo = float(daily["ci95_low"])
    daily_hi = float(daily["ci95_high"])
    weekly_lo = float(weekly["ci95_low"])
    weekly_hi = float(weekly["ci95_high"])
    shift = weekly_est - daily_est
    fig, ax = plt.subplots(figsize=(PAIR_WIDTH_IN, SHORT_HEIGHT_IN))
    fig.subplots_adjust(left=0.20, right=0.98, top=0.96, bottom=0.20)
    daily_face = tuple(
        0.70 * np.array(matplotlib.colors.to_rgb(BLUE))
        + 0.30 * np.array(matplotlib.colors.to_rgb(LIGHT_GREY))
    )
    ax.axvspan(-0.55, 0.5, color=LIGHT_GREY, alpha=0.9, zorder=0)
    ax.axvspan(0.5, 1.5, color=LIGHT_AMBER, alpha=0.7, zorder=0)
    ax.axvspan(1.5, 2.55, color=LIGHT_BLUE, alpha=0.55, zorder=0)
    width = 0.55
    ax.bar(0, daily_est, color=daily_face, width=width, zorder=2)
    ax.bar(1, shift, bottom=daily_est, color=AMBER, width=width, zorder=2)
    ax.bar(2, weekly_est, color=BLUE, width=width, zorder=2)
    cap = dict(color=INK, capsize=1.8, elinewidth=0.85, linewidth=0.85, zorder=3)
    ax.errorbar(0, daily_est, yerr=[[daily_est - daily_lo], [daily_hi - daily_est]], **cap)
    ax.errorbar(2, weekly_est, yerr=[[weekly_est - weekly_lo], [weekly_hi - weekly_est]], **cap)
    ax.axhline(0, color=INK, linewidth=0.7)
    ax.text(0, daily_hi + 0.05, f"+{daily_est:.3f}", ha="center", va="bottom", color=MUTED, fontsize=PAIR_NOTE_PT, fontweight="bold", clip_on=False)
    ax.text(1, weekly_est + 0.04, f"+{shift:.3f}", ha="center", va="bottom", color=AMBER, fontsize=PAIR_NOTE_PT, fontweight="bold", clip_on=False)
    ax.annotate(
        f"+{weekly_est:.3f}",
        xy=(2, weekly_hi),
        xytext=(-5, 0),
    ...
    ax.set_xticks([0, 1, 2], ["Daily\nestimate", "Aggregation\nshift", "Weekly\nestimate"])
    ax.set_ylabel("Effect on\nlog(1 + volume)", linespacing=1.12)
    ax.yaxis.labelpad = 2.2
    ax.set_xlim(-0.55, 2.55)
    ax.set_ylim(-0.22, 1.58)
    ax.set_yticks([0.0, 0.5, 1.0, 1.5])
    _pair_axes(ax)
    save(fig, "fig_frequency_sensitivity", native=True)


def make_mechanism() -> None:
    h1 = load_json("application/h1_rpc_mechanism_summary.json")
    fig, ax = plt.subplots(figsize=(APP_WIDTH_IN, SHORT_HEIGHT_IN))
    fig.subplots_adjust(left=0.345, right=0.72, top=0.96, bottom=0.22)
    ...
    colors = [TEAL, GREEN]
    ys = [0.50, 0.0]
    ax.barh(ys, nums, color=colors, height=0.32, edgecolor="none", zorder=2)
    for y, val, count, color in zip(ys, nums, counts, colors):
        ax.text(1.04, y + 0.065, f"{100 * val:.1f}%", va="center", ha="left", fontsize=PAIR_LABEL_PT, color=color, fontweight="bold", clip_on=False, zorder=4)
        ax.text(1.04, y - 0.07, f"({count})", va="center", ha="left", fontsize=PAIR_NOTE_PT, color=MUTED, clip_on=False, zorder=4)
    ax.set_xlim(0, 1.10)
    ...
    save(fig, "fig_h1_mechanism_audit", native=True)


def make_agentic() -> None:
    d = read_csv("application/agentic_arm_scores.csv")
    fig, ax = plt.subplots(figsize=(APP_WIDTH_IN, SHORT_HEIGHT_IN))
    fig.subplots_adjust(left=0.165, right=0.620, top=0.96, bottom=0.16)
    x = np.arange(len(d))
    ...
    ax.text(7.48, float(omit.iloc[-1]) + 0.012, "Method\nomission rate", color=AMBER, fontsize=PAIR_NOTE_PT, ha="left", va="bottom", linespacing=1.12, clip_on=False)
    ax.text(7.48, float(gap.iloc[-1]) - 0.012, "Absolute\ncalibration gap", color=BLUE, fontsize=PAIR_NOTE_PT, ha="left", va="top", linespacing=1.12, clip_on=False)
    ax.set_xticks(x, d["rung"])
    ax.set_yticks([0.2, 0.4, 0.6, 0.8])
    ax.set_xlim(-0.45, 7.22)
