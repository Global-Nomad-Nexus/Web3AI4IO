...
def build_teacher_requirements_alignment(config: CaseConfig, mechanism_summary: dict[str, object]) -> pd.DataFrame:
    """Map revision requirements to local code/artifact status."""
    ...
    rows = [
        ...
        },
        {
            ...
            "artifact": str(config.project_root / "README.md"),
            "status": "added_to_report_needs_joint_paper_sync",
            "evidence_or_gap": "Application-arm documentation includes retail-user protection, financial inclusion, welfare-overclaim, SDG 8/10, and public-good benchmark framing; joint paper text still needs synchronization.",
        },
        {
            "ownership_item": "Joint August 7 deliverable scope",
            "teacher_requirement": "Provide revised dataset specification, cross-chain extension, application-arm mirror case ladder, AI-for-good framing, artifact outline, and remaining blockers.",
            "artifact": str(config.project_root / "benchmark_release" / "README.md"),
            "status": "application_side_release_candidate_with_blockers",
            "evidence_or_gap": "Application-arm artifacts are concrete; identification-arm semi-synthetic suite and joint paper integration remain outside this application-only audit.",
        },
        {
            "ownership_item": "H1 migration friction / post-graduation persistence",
            "teacher_requirement": "The application arm owns H1 and Result 1 mechanism evidence.",
        ...
        },
        {
            ...
            "teacher_requirement": "The application arm owns H4; H2 is joint.",
        ...
        {
            "ownership_item": "GitHub/application folder discipline",
            "teacher_requirement": "Put code, data, and artifacts under the application-arm folder.",
            "artifact": str(config.output_root),
            "status": "computed",
            "evidence_or_gap": "All new causal-validation artifacts are generated under application/artifacts.",
        },
    ]
    alignment = pd.DataFrame(rows)
    write_csv(tables / "teacher_requirements_alignment.csv", alignment)
    return alignment
