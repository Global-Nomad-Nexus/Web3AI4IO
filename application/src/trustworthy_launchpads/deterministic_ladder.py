        ...
        rows.append(
            {
                ...
                "interpretation": (
                    "Package-level DiD cross-check. The application arm uses this for reproducibility; "
                    "the identification arm owns the full staggered cross-chain DiD estimator."
                ),
            }
        )
...
def estimate_l3(event: pd.DataFrame, fit: object) -> dict[str, object]:
    ...
    return {
        ...
        "method": "Single-event dynamic TWFE/event-study proxy; staggered rung is partially vacuous for this application case.",
        "notes": "The identification arm owns true staggered LP-DiD. Here L3 records the compatible single-event dynamic analogue.",
    }
        ...
        rows.append(
            {
                ...
                "interpretation": (
                    ...
                    "this is outcome measurement for the application H1 layer, not the cross-chain staggered DiD."
                ),
            }
        )
    else:
...
def run_ladder(config: CaseConfig) -> LadderOutputs:
    ...
    write_csv(config.tables_dir / "event_study_coefficients.csv", event)
