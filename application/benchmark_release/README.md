...
This directory is the application-arm release candidate for the Web3AI4IO benchmark after the August 7 revision plan. It only covers the Pump.fun / PumpSwap application arm and does not include the identification-arm semi-synthetic suite.

## Primary Sheets

| Sheet | Role |
|---|---|
| `data/events.csv` | Rule-event and candidate-event registry for the application case, including Solana PumpSwap, accepted matched Clanker/Base, and rejected candidates. |
...
| `data/mirror_case_candidates.csv` | Current application-arm mirror-case scan. The Telegram heterogeneity case is a credible matched signal but not causal yet. |
| `data/mirror_case_ladder.csv` | A rung-by-rung Case B ladder for the Telegram/social metadata candidate. |
| `data/paired_case_ladder.csv` | Same-ladder Case A/Case B rows behind `artifacts/figures/fig_paired_case_ladder.png`. |
...
| `data/teacher_requirements_alignment.csv` | Application-arm audit against the revision requirements. |
