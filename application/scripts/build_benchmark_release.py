#!/usr/bin/env python3
"""Build application's benchmark release tables for the PumpSwap application arm."""
    ...
    if clanker_summary.get("status", "").startswith("accepted"):
        ...
        clanker_event_row = {
            ...
            "activation_evidence": (
                ...
                "TokenCreated queries. Use as application's highest-priority cross-chain candidate, not as computed evidence."
            ),
            "activation_transaction_hash": "",
            "next_block_verification": "",
            "anticipation_boundary_utc": "",
...
def build_claim_scope() -> pd.DataFrame:
    ...
    cross_chain_allowed = (
        ...
        if clanker_summary.get("status", "").startswith("accepted")
        else "Clanker/Base is the highest-priority candidate for application's cross-chain extension."
    )
        ...
        [
            {
                ...
                "comparability_to_application_schema": "event_id, token_id, timestamp, 1/7/30d outcomes, early-wallet concentration, claim_boundary",
                "priority": "high",
            ...
            },
            {
                ...
                "comparability_to_application_schema": "Same event registry and fixed token horizons if a dated rule event is found.",
                "priority": "medium",
            ...
            },
            {
                ...
                "current_evidence": "Project notes list it as a target platform; no event accepted.",
                "comparability_to_application_schema": "Same event registry and fixed token horizons if transaction coverage is available.",
                "priority": "medium",
                "status": "discovery_needed",
...
def build_teacher_requirements_alignment() -> pd.DataFrame:
    return read_csv(ROOT / "artifacts" / "tables" / "teacher_requirements_alignment_application.csv")
...
def build_data_dictionary() -> pd.DataFrame:
    rows = [
        ...
        ("teacher_requirements_alignment_application.csv", "status", "category", "application-only status against Luyao's revision requirements."),
...
def write_manifest(paths: list[Path]) -> None:
    ...
    write_json(
        DATA / "release_manifest.json",
        {
            ...
            "scope": "application Pump.fun/PumpSwap application arm only",
            ...
            "supplemental_sheets": [
                ...
                "teacher_requirements_alignment_application.csv",
...
def main() -> None:
    ...
    outputs = {
        ...
        "teacher_requirements_alignment_application.csv": (build_teacher_requirements_alignment(), None),
