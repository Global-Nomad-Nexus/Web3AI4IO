#!/usr/bin/env python3
"""Build application-arm benchmark release tables for the PumpSwap application arm."""
...
CONTEXT_EVENT_ID = "APPLICATION_OFFCHAIN_CONTEXT_PANEL"
    ...
    if clanker_summary.get("status", "").startswith("accepted"):
        ...
        clanker_event_row = {
            ...
            "activation_evidence": (
                ...
                "TokenCreated queries. Use as the application arm's highest-priority cross-chain candidate, not as computed evidence."
            ),
            "activation_transaction_hash": "",
            "next_block_verification": "",
            "anticipation_boundary_utc": "",
        ...
        rows.append(
            {
                ...
                "source_layer": "application/artifacts/external_validation/solana_post_migration_pool_windows.csv",
        ...
        rows.append(
            {
                ...
                "source_layer": "application/artifacts/external_validation/moralis_decoded_token_outcomes.csv",
        ...
        rows.append(
            {
                ...
                "source_layer": "application/artifacts/external_validation/dune_early_wallets.csv",
        ...
        rows.append(
            {
                ...
                "source_layer": "application/artifacts/external_validation/clanker_base_token_horizons.csv",
...
def build_claim_scope() -> pd.DataFrame:
    ...
    cross_chain_allowed = (
        ...
        if clanker_summary.get("status", "").startswith("accepted")
        else "Clanker/Base is the highest-priority candidate for the application-arm cross-chain extension."
    )
    ...
    rows.extend(
        [
            {
                "claim_id": "H1-benchmark-release-application",
                "hypothesis": "H1 migration friction",
                "evidence_layer": "Application three-sheet benchmark release",
                "status": "release_candidate",
                "claim_allowed": "The application package exposes events, metrics, covariates, claim boundaries, and data gaps as reusable benchmark artifacts.",
            ...
            {
                "claim_id": "cross-chain-application",
        ...
        [
            {
                ...
                "comparability_to_schema": "event_id, token_id, timestamp, 1/7/30d outcomes, early-wallet concentration, claim_boundary",
                "priority": "high",
            ...
            },
            {
                ...
                "comparability_to_schema": "Same event registry and fixed token horizons if a dated rule event is found.",
                "priority": "medium",
            ...
            },
            {
                ...
                "current_evidence": "Project notes list it as a target platform; no event accepted.",
                "comparability_to_schema": "Same event registry and fixed token horizons if transaction coverage is available.",
                "priority": "medium",
                "status": "discovery_needed",
        ...
        rows.append(
            {
                ...
                "figure_artifact": "artifacts/figures/fig_paired_case_ladder.png",
            }
        )
    return pd.DataFrame(rows)
...
def build_teacher_requirements_alignment() -> pd.DataFrame:
    return read_csv(ROOT / "artifacts" / "tables" / "teacher_requirements_alignment.csv")
...
def build_data_dictionary() -> pd.DataFrame:
    rows = [
        ...
        ("teacher_requirements_alignment.csv", "status", "category", "Application-arm status against the revision requirements."),
...
def build_requirement_closure_audit() -> pd.DataFrame:
    ...
    specs = [
        ...
        (
            ...
            "Reference artifacts/figures/fig_paired_case_ladder.png in the joint draft.",
        ),
        (
...
def build_zenodo_metadata() -> dict[str, Any]:
    return {
        "title": "Web3AI4IO PumpSwap Application Benchmark Release",
        "upload_type": "dataset",
        "description": (
            "Machine-readable evidence-quality benchmark artifacts for the Pump.fun/PumpSwap application arm, "
...
def write_manifest(paths: list[Path]) -> None:
    ...
    write_json(
        DATA / "release_manifest.json",
        {
            ...
            "scope": "Pump.fun/PumpSwap application arm only",
            ...
            "supplemental_sheets": [
                ...
                "teacher_requirements_alignment.csv",
...
def main() -> None:
    ...
    outputs = {
        ...
        "teacher_requirements_alignment.csv": (build_teacher_requirements_alignment(), None),
    ...
    print(f"Application benchmark release written to {DATA}")
    print(
