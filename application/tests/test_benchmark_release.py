...
@unittest.skipUnless(_LOCAL_BENCHMARK, "local application benchmark tables are not part of the GitHub tree")
class BenchmarkReleaseTest(unittest.TestCase):
    ...
    def test_release_scope_is_application_only(self) -> None:
        manifest = json.loads((DATA / "release_manifest.json").read_text(encoding="utf-8"))
        self.assertIn("application", manifest["scope"])
    ...
    def test_teacher_requirements_alignment_covers_revision_email(self) -> None:
        alignment = pd.read_csv(DATA / "teacher_requirements_alignment_application.csv")
        required_items = {
