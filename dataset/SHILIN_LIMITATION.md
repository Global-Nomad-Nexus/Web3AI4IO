# Shilin Data Limitation

## Scope

This note documents the status and permitted use of the decoded Solana swap data supplied in the Shilin reproducibility bundle.
...
The decoded swap files are retained exactly as supplied by Shilin and normalized into the canonical release without imputing missing pages or tokens.
...
The data expansion project accepts this limitation. It will not continue collecting decoded Solana swaps. Shilin's delivered files remain the final decoded swap snapshot for this project.

Future canonical releases must preserve the following rules:

1. Keep the raw Shilin files immutable.
...
The canonical `decoded_swaps` and `token_horizons` tables preserve the Shilin snapshot. The `coverage_ledger` records 294 tokens as `decoded_lower_bound_page_capped` and 1,357 tokens as `no_decoded_swap_data`. No additional decoded swap acquisition is planned.
