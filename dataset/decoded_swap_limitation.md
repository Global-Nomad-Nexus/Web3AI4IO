# Decoded swap limitation

## Scope

This note documents the status and permitted use of the decoded Solana swap data supplied in the application-arm reproducibility bundle.
...
The decoded swap files are retained exactly as supplied in the application-arm bundle and normalized into the canonical release without imputing missing pages or tokens.
...
The data expansion project accepts this limitation. It will not continue collecting decoded Solana swaps. The delivered application-arm files remain the final decoded swap snapshot for this project.

Future canonical releases must preserve the following rules:

1. Keep the raw application-arm bundle files immutable.
...
The canonical `decoded_swaps` and `token_horizons` tables preserve the delivered snapshot. The `coverage_ledger` records 294 tokens as `decoded_lower_bound_page_capped` and 1,357 tokens as `no_decoded_swap_data`. No additional decoded swap acquisition is planned.
