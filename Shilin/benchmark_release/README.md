# Shilin Benchmark Release

This directory is Shilin's release candidate for the Web3AI4IO benchmark after the August 7 revision plan. It only covers the Pump.fun / PumpSwap application arm and does not include Claire's semi-synthetic suite.

## Primary Sheets

| Sheet | Role |
|---|---|
| `data/events.csv` | Rule-event and candidate-event registry for Shilin's case, including Solana PumpSwap, accepted matched Clanker/Base, and rejected candidates. |
...
| `data/mirror_case_candidates.csv` | Shilin's current mirror-case scan. The Telegram heterogeneity case is a credible matched signal but not causal yet. |
| `data/mirror_case_ladder.csv` | A rung-by-rung Case B ladder for the Telegram/social metadata candidate. |
| `data/paired_case_ladder.csv` | Same-ladder Case A/Case B rows behind `artifacts/figures/fig_paired_case_ladder_shilin.png`. |
...
| `data/full_cohort_coverage_audit.csv` | Processed-unit shares and closure requirements for Base full-cohort archive/indexer coverage. |
| `data/clanker_base_causal_diagnostics.csv` | Matched-pair Base v4.1 versus v4.0 diagnostics for the currently covered sample. |
| `data/teacher_requirements_alignment_shilin.csv` | Shilin-only audit against Luyao's revision requirements. |
| `data/requirement_closure_audit.csv` | Requirement-level closure status separating workshop-ready items from top-conference gaps. |
| `data/top_conference_gap_ledger.csv` | Concrete remaining experiments or credentials needed for stronger causal and benchmark claims. |
| `data/agentic_evaluation_panel.csv` | L0-L7 agentic scores merged with deterministic ladder outputs and prompt hashes. |
| `data/agentic_multimodel_ablation_manifest.csv` | Registered baseline and leave-one-scaffold-out ablation cells. |
| `data/agentic_multimodel_ablation_scores.csv` | Scored API and no-API Ollama model outputs for the registered ablation cells. |
...
| `zenodo_metadata.json` | Zenodo-ready deposit metadata; DOI minting remains an external release step. |

## Current Shilin Direction
...
The H4 early-wallet extension is now a bounded decoded-proxy sample rather than a placeholder. The checked-in Solana RPC backfill covers 10 graduated tokens, 298 early parsed pool transactions, and 101 conservative buyer/seller/holder proxy classifications from token-balance deltas. It records 53 decoded buyer-proxy wallets and 60 decoded holder-proxy wallets. This remains a sample-level mechanism artifact until every one of the 1,651 graduated PumpSwap tokens has decoded buyer/holder coverage.

The agentic benchmark now includes multi-model scaffold ablations. The release has 65 scored ablation cells and 105 successful outputs across DeepSeek `deepseek-chat`, Ollama `llama3.1`, Ollama `deepseek-r1:8b`, and Ollama `qwen2.5:0.5b`. Failed model cells are retained as audit rows, and ablation differences should be interpreted as benchmark behavior, not causal treatment effects of prompt scaffolds.
