...
def export_tab_ablation(ladder: pd.DataFrame, path: Path) -> None:
    lines = [
        "% Auto-generated by Shilin/scripts/run_all.py",
        "\\begin{table}[t]",
        "\\centering",
        "\\footnotesize",
        "\\caption{Shilin PumpSwap ablation ladder. Each row adds one component to the evidence pipeline.}",
        "\\label{tab:ablation-shilin}",
        "\\begin{tabularx}{\\textwidth}{@{}l X c c c@{}}",
        "\\toprule",
        "\\textbf{Rung} & \\textbf{Component added} & \\textbf{Estimate} & \\textbf{CI} & \\textbf{Conclusion} \\\\",
        "\\midrule",
    ]
    for row in ladder.itertuples(index=False):
        ci = f"[{_fmt_num(row.ci95_low)}, {_fmt_num(row.ci95_high)}]"
        lines.append(
            f"{row.rung} & {row.component_added} & {_fmt_num(row.estimate)} & {ci} & {row.worked_decision} \\\\"
        )
