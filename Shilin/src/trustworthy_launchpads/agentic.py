...
def export_tab_arms(deterministic: pd.DataFrame, scores: pd.DataFrame, path: Path) -> None:
    scores = scores.set_index("rung")
    scored_all = len(scores) > 0 and bool(scores["status"].eq("scored").all())
    caption = (
        "Shilin execution-arm ledger with DeepSeek repeated agent runs."
        if scored_all
        else "Shilin execution-arm ledger. Agentic columns remain pending until registered model runs are executed."
    )
    lines = [
        ...
        "\\footnotesize",
        f"\\caption{{{caption}}}",
        "\\label{tab:arms-shilin}",
        "\\begin{tabular}{@{}clcccc@{}}",
        "\\toprule",
        " & & \\multicolumn{2}{c}{\\textbf{Conclusion}} & \\multicolumn{2}{c}{\\textbf{Agentic arm only}} \\\\",
        "\\cmidrule(lr){3-4}\\cmidrule(lr){5-6}",
        "\\textbf{Rung} & \\textbf{Component added} & \\textbf{Deterministic} & \\textbf{Agent} & \\textbf{Sign stab.} & \\textbf{Cal. gap / disp.} \\\\",
        "\\midrule",
    ]
    for row in deterministic.itertuples(index=False):
        score = scores.loc[row.rung] if row.rung in scores.index else None
        if score is None or score["status"] != "scored":
            agent = "[registered, not run]"
            sign = "[pending]"
            gap = "[pending]"
        else:
            agent = str(score["agent_mean_decision"])
            sign = f"{float(score['sign_stability']):.2f}"
            gap = f"{float(score['calibration_gap']):.2f} / {float(score['run_to_run_dispersion']):.3f}"
        lines.append(
            f"{row.rung} & {row.component_added[:38]} & {row.worked_decision} & {agent} & {sign} & {gap} \\\\"
        )
