#!/usr/bin/env python3
"""Render or execute full token-level Dune exports for Shilin H1/H4.
...
def render_post_migration_sql(df: pd.DataFrame) -> str:
    ...
    return f"""-- Auto-rendered by Shilin/scripts/run_dune_token_exports.py.
    ...
    *,
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "ShilinResearchBot/0.1",
        },
    )
    try:
...
def download_csv(
    ...
    *,
    ...
    request = urllib.request.Request(
        url,
        method="GET",
        headers={"X-Dune-Api-Key": api_key, "User-Agent": "ShilinResearchBot/0.1"},
    )
    try:
        with urllib.request.urlopen(request, timeout=timeout) as response:
