        ...
        rows.append(
            {
                ...
                "interpretation": (
                    "Package-level DiD cross-check. application uses this for reproducibility; "
                    "identification owns the full staggered cross-chain DiD estimator."
                ),
            }
        )
...
def estimate_l3(event: pd.DataFrame, fit: object) -> dict[str, object]:
    ...
    return {
        ...
        "method": "Single-event dynamic TWFE/event-study proxy; staggered rung is partially vacuous for application case.",
        "notes": "identification owns true staggered LP-DiD. Here L3 records the compatible single-event dynamic analogue.",
    }
        ...
        rows.append(
            {
                ...
                "interpretation": (
                    ...
                    "this is outcome measurement for application's H1 layer, not the cross-chain staggered DiD."
                ),
            }
        )
    else:
...
def run_ladder(config: CaseConfig) -> LadderOutputs:
    ...
    write_csv(config.tables_dir / "event_study_coefficients_application.csv", event)
