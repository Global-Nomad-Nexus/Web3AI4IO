# Claire study research design

Status: approved design direction, 2026-08-02

This study starts from the current paper in `web3IO/` and the assignment in `records/emails/0730-did-improve.docx`. The previous MVP is excluded from the research design. It is not a baseline, data source, event source, result source, or prior for expected findings.
...
Claire's contribution has two linked roles.
...
## Claire deliverables mapped to the design
...
4. The independent Claire draft of Section 2.2 specifies the identification ladder, measurement axis, real world cases, and known truth calibration boundary.
5. Result 2 reports the event registry, H0 estimates, H3 estimates, sequential event handling, and robustness envelope.
6. Result 3 contributes the Claire ladder, deterministic results, and component omission analysis.
